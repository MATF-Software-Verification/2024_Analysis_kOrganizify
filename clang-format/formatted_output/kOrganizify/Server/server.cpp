...
void Server::readFromClient() {
  ...
  if (title == "new connection") {
    ...
    newClientMessage.insert("fromUsername",
                            doc.value("fromUsername").toString());
    ...
    for (const QJsonValue &jv : jsonArray) {
      ...
      event.setStartTime(
          QDateTime::fromString(jv["startTime"].toString(), Qt::ISODate));
      event.setEndTime(
          QDateTime::fromString(jv["endTime"].toString(), Qt::ISODate));
...
void Server::sendEvent(int ind) const {
  ...
  newSyncEventMsg.insert("startTime",
                         m_currentSyncEvents[ind].getStartTime().toString());
...
void Server::sendFinalEvent(int ind) const {
  ...
  newSyncEventMsg.insert("startTime",
                         m_currentSyncEvents[ind].getStartTime().toString());
  newSyncEventMsg.insert("endTime",
                         m_currentSyncEvents[ind].getEndTime().toString());
  newSyncEventMsg.insert("eventTitle", m_currentSyncEvents[ind].getTitle());
...
void Server::sendToClient(const QString &username,
...
QList<Event> Server::findFreeTime(Calendar *cal1, Calendar *cal2,
                                  int maxTime) const {
  ...
  auto isBetween12pmAnd8am = [](const QTime &currentHour) {
    return (currentHour >= QTime(0, 0) && currentHour < QTime(12, 0)) ||
           (currentHour >= QTime(12, 0) && currentHour <= QTime(8, 0));
  };
  ...
  for (QDate currentDate = currentDay; currentDate <= lastDayOfWeek;
       currentDate = currentDate.addDays(1)) {
    QTime startHour = (currentDate == currentDay)
                          ? roundUpToNextHour(currentHour)
                          : QTime(8, 0);
    QTime endHour = QTime(23, 59, 59);

    if (isBetween12pmAnd8am(startHour)) startHour = QTime(8, 0);

    for (QTime currentHour = startHour; currentHour < endHour;
         currentHour = currentHour.addSecs(3600)) {
      ...
      if (currentHour <= endTime) {
        ...
        newEvent->setEndTime(
            QDateTime(currentDate, currentHour.addSecs(maxTime * 3600)));
        newEvent->setTitle(m_syncEventTitle);

        freeTimeSlots.append(*newEvent);
      }

      if (currentHour.hour() == 23) break;
    }
  }

  // removing free time slots that overlap with existing events
  auto it = std::remove_if(
      freeTimeSlots.begin(), freeTimeSlots.end(),
      [&](const Event &freeTimeSlot) {
