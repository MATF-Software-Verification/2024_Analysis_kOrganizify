...
TEST_CASE("customEventPriorityToString(CustomEventPriority priority)",
          "[Event]") {
  SECTION("Tacno konvertuje prioritet u string") {
    const CustomEventPriority priorityToTest = CustomEventPriority::NoPriority;
    const QString convertedPriority =
        customEventPriorityToString(priorityToTest);
    REQUIRE(convertedPriority == "No Priority");
  }
}

TEST_CASE("customEventPriorityFromString(const QString &priorityString)",
          "[Event]") {
  SECTION("Tacno konvertuje string u enum prioritet") {
    const QString priorityStringToTest = "Low";
    const CustomEventPriority convertedPriority =
        customEventPriorityFromString(priorityStringToTest);
