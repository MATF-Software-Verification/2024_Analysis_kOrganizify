...
void Scheduler::generateSchedule(const QTime &startOfWorkday,
                                 const QTime &endOfWorkday) {
  ...
  std::sort(m_allEvents.begin(), m_allEvents.end(),
            [](const BasicEvent &a, const BasicEvent &b) {
...
void Scheduler::generatePermutations(QList<Event> &events, int start, int end,
                                     int depth,
                                     QList<QList<Event>> &allPermutations) {
  if (depth >= 1) {
...
QList<Event> Scheduler::findFreeTime(Calendar *cal1, int maxTimeInMinutes) {
  ...
  for (QDate currentDate = currentDay; currentDate <= lastDayOfWeek;
    ...
    for (QTime currentHour = startHour; currentHour < endHour;
         currentHour = currentHour.addSecs(60 * maxTimeInMinutes)) {
      ...
      if (currentHour <= endTime) {
        ...
        newEvent->setEndTime(
            QDateTime(currentDate, currentHour.addSecs(60 * maxTimeInMinutes)));

        freeTimeSlots.append(*newEvent);
      }

      if (currentHour.addSecs(60 * maxTimeInMinutes).hour() == 23) break;
    }
  }
