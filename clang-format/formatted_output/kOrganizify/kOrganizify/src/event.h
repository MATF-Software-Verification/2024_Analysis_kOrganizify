...
CustomEventPriority customEventPriorityFromString(
    const QString &priorityString);
...
class Event : public BasicEvent {
 ...
 private:
  ...
  friend uint qHash(const Event &event) {
    return qHash(event.getTitle()) ^ qHash(event.getStartTime()) ^
           qHash(event.getEndTime());
  }
};
