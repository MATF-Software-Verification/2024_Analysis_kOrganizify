...
void Client::readFromServer() {
  ...
  if (title == "new connection") {
    ...
    event.setStartTime(
        QDateTime::fromString(doc.value("startTime").toString(), Qt::ISODate));

    emit newEventSync(doc.value("eventTitle").toString(),
                      doc.value("startTime").toString());
  } else if (title == "agreed sync") {
    QDateTime startTime = QDateTime::fromString(
        doc.value("startTime").toString());  // won't convert
...
void Client::syncResponse(bool response, QString username, QString friendName,
...
void Client::syncRequest(QString from, QString to, QString titleEvent,
                         int duration, Calendar calendar) {
  QJsonObject responseMsg;
